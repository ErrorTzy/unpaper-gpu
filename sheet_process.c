...
bool process_sheet(SheetProcessState *state, const SheetProcessConfig *config) {
  ...
  for (int j = 0; j < state->input_count; j++) {
    if (state->input_files[j] != NULL ||
      ...
      state->input_size = coerce_size(
          state->input_size,
          coerce_size(options->sheet_size, inputSheetSize));
    } else {
      state->page = EMPTY_IMAGE;
    }
    ...
    if (state->page.frame != NULL) {
      center_image(state->page, state->sheet,
  ...
  // Stretch
  state->input_size = coerce_size(options->stretch_size, size_of_image(state->sheet));
  state->input_size.width *= options->pre_zoom_factor;
  state->input_size.height *= options->pre_zoom_factor;
  stretch_and_replace(&state->sheet, state->input_size, options->interpolate_type);

  // Resize if needed
  if (options->page_size.width != -1 || options->page_size.height != -1) {
    state->input_size = coerce_size(options->page_size, size_of_image(state->sheet));
    resize_and_replace(&state->sheet, state->input_size, options->interpolate_type);
  }
  ...
  if (options->layout == LAYOUT_SINGLE) {
    if (state->point_count == 0) {
      state->points[state->point_count++] =
          (Point){state->sheet.frame->width / 2, state->sheet.frame->height / 2};
    }
    ...
    if (state->point_count == 0) {
      state->points[state->point_count++] =
          (Point){state->sheet.frame->width / 4, state->sheet.frame->height / 2};
      state->points[state->point_count++] =
    ...
    if (state->outside_borderscan_mask_count == 0) {
      state->outside_borderscan_mask[state->outside_borderscan_mask_count++] =
          (Rectangle){{POINT_ORIGIN,
                       {state->sheet.frame->width / 2, state->sheet.frame->height - 1}}};
      state->outside_borderscan_mask[state->outside_borderscan_mask_count++] =
          (Rectangle){{{state->sheet.frame->width / 2, 0},
                       {state->sheet.frame->width - 1, state->sheet.frame->height - 1}}};
    }
  }
  ...
  // Pre-wipe
  if (!isExcluded(nr, options->no_wipe_multi_index, options->ignore_multi_index)) {
    apply_wipes(state->sheet, options->pre_wipes, options->mask_color);
  }

  // Pre-border
  if (!isExcluded(nr, options->no_border_multi_index, options->ignore_multi_index)) {
  ...
  if (config->blackfilter_exclude_count == 0 && options->layout != LAYOUT_NONE) {
    ...
    if (options->layout == LAYOUT_SINGLE) {
      bf_params.exclusions[bf_params.exclusions_count++] = rectangle_from_size(
          ...
          (RectangleSize){.width = sheetSize.width / 2, .height = sheetSize.height / 2});
    } else if (options->layout == LAYOUT_DOUBLE) {
      ...
      Point secondFilterOrigin =
          shift_point(firstFilterOrigin, (Delta){state->sheet.frame->width / 2});
  ...
  if (state->mask_count > 0) {
    apply_masks(state->sheet, state->masks, state->mask_count, options->mask_color);
  }
  ...
  // Deskew
  if (!isExcluded(nr, options->no_deskew_multi_index, options->ignore_multi_index)) {
    ...
    if (!isExcluded(nr, options->no_mask_scan_multi_index,
                    options->ignore_multi_index)) {
      state->mask_count =
          detect_masks(state->sheet, mask_params, state->points,
    ...
    for (size_t i = 0; i < state->mask_count; i++) {
      ...
      if (rotation != 0.0) {
        deskew(state->sheet, state->masks[i], rotation, options->interpolate_type);
      }
    }
  ...
  if (!isExcluded(nr, options->no_border_scan_multi_index,
    ...
    apply_masks(state->sheet, autoborderMask, state->outside_borderscan_mask_count,
                options->mask_color);
    for (size_t i = 0; i < state->outside_borderscan_mask_count; i++) {
  ...
  // Post-shifting
  if (options->post_shift.horizontal != 0 || options->post_shift.vertical != 0) {
  ...
  // Post-stretch
  state->input_size = coerce_size(options->post_stretch_size, size_of_image(state->sheet));
  state->input_size.width *= options->post_zoom_factor;
  state->input_size.height *= options->post_zoom_factor;
  stretch_and_replace(&state->sheet, state->input_size, options->interpolate_type);

  // Post-size
  if (options->post_page_size.width != -1 || options->post_page_size.height != -1) {
    state->input_size = coerce_size(options->post_page_size, size_of_image(state->sheet));
    resize_and_replace(&state->sheet, state->input_size, options->interpolate_type);
  }
