...
bool unpaper_npp_integral_8u32s(uint64_t src_device, int src_width,
  ...
  uint64_t padded_src_device = 0;

  cudaError_t err = cudaMalloc((void **)&padded_src_device, padded_src_bytes);
  if (err != cudaSuccess || padded_src_device == 0) {
    errOutput("NPP integral: failed to allocate padded input buffer");
    return false;
  }

  // Initialize padded buffer to zero (for the padding)
  err = cudaMemset((void *)padded_src_device, 0, padded_src_bytes);
  if (err != cudaSuccess) {
    errOutput("NPP integral: failed to zero padded input buffer");
    cudaFree((void *)padded_src_device);
    return false;
  }
  ...
  NppStatus status = nppiIntegral_8u32s_C1R_Ctx(
      ...
      (Npp32s *)dst_device, (int)dst_step,
      roi, 0, *npp_ctx);

  // Free padded input buffer
  cudaFree((void *)padded_src_device);
...
int64_t unpaper_npp_integral_rect_sum(const UnpaperNppIntegral *integral,
  ...
  // - Original image max coords: orig_w-1 = integral->width-2, orig_h-1 = integral->height-2
