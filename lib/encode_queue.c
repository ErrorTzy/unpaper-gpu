...
static bool encode_direct(const char *filename, AVFrame *frame, int output_format) {
...
static bool encode_ffmpeg(const char *filename, AVFrame *input_frame,
  ...
  if (input_frame->format != output_format) {
    ...
    if (input_frame->format == AV_PIX_FMT_RGB24 &&
        output_format == AV_PIX_FMT_MONOWHITE) {
      for (int y = 0; y < height; y++) {
        const uint8_t *src = input_frame->data[0] + y * input_frame->linesize[0];
  ...
  while (1) {
    ...
    if (job->valid && job->frame) {
      if (job->output_count == 1) {
        ...
        encode_ffmpeg(job->output_files[0], job->frame, job->output_pixel_format);
      } else {
            ...
            if (av_frame_get_buffer(sub_frame, 0) >= 0) {
              ...
              if (job->frame->format == AV_PIX_FMT_RGB24) {
                ...
                for (int y = 0; y < job->frame->height; y++) {
                  const uint8_t *src_row = job->frame->data[0] + y * job->frame->linesize[0];
                  uint8_t *dst_row = sub_frame->data[0] + y * sub_frame->linesize[0];
                ...
                encode_ffmpeg(job->output_files[j], sub_frame, job->output_pixel_format);
              ...
              for (int y = 0; y < job->frame->height; y++) {
                const uint8_t *src = job->frame->data[0] +
                    y * job->frame->linesize[0] + output_width * j * bytes_per_pixel;
...
bool encode_queue_start(EncodeQueue *queue) {
  ...
  for (int i = 0; i < queue->num_encoder_threads; i++) {
    if (pthread_create(&queue->encoder_threads[i], NULL, encoder_thread_fn, queue) != 0) {
...
bool encode_queue_submit(EncodeQueue *queue, struct AVFrame *frame,
  ...
  slot->job.output_count = output_count > ENCODE_MAX_OUTPUTS ? ENCODE_MAX_OUTPUTS : output_count;
...
EncodeQueueStats encode_queue_get_stats(const EncodeQueue *queue) {
  ...
  if (stats.images_encoded > 0) {
    stats.avg_encode_time_ms = stats.total_encode_time_ms / (double)stats.images_encoded;
  }
...
void encode_queue_print_stats(const EncodeQueue *queue) {
  ...
  if (stats.images_queued > 0) {
    producer_wait_rate = 100.0 * (double)stats.producer_waits / (double)stats.images_queued;
  }
  if (stats.images_encoded > 0) {
    consumer_wait_rate = 100.0 * (double)stats.consumer_waits / (double)stats.images_encoded;
  }
  ...
  fprintf(stderr,
          ...
          queue->queue_depth, queue->num_encoder_threads,
          stats.images_queued, stats.images_encoded,
